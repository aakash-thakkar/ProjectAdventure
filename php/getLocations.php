...
$sortIndex = $_POST['sortParameter'];
if($sortIndex == 0)
{
	$query="Select * from wildr.weather,wildr.Location,wildr.distance where weather.Location_location_Id=location.location_id AND location.location_id=distance.Location_location_id ;";
}
else if($sortIndex==1)
{
	$query="Select * from wildr.weather,wildr.Location,wildr.distance where weather.Location_location_Id=location.location_id AND location.location_id=distance.Location_location_id ORDER BY distance.Distance_value ASC ;";
}
else
{
	$query="Select * from wildr.weather,wildr.Location,wildr.distance where weather.Location_location_Id=location.location_id AND location.location_id=distance.Location_location_id ORDER BY distance.Distance_value DESC ;";
...
while ($row = mysqli_fetch_assoc($runq))
	...
	echo " <div class='list-item-division'><ul><li>" . $row['Distance_value'] . " km</li>";
